...
2. **实时写入**：Consumer → MySQL `energy_record`
3. **实时预警**：Alert Engine 消费 Kafka，超阈值写入 MySQL `alert_record`
4. **离线归档**：Flume/Spark → HDFS → Hive DWD 层
5. **展示**：Vue 前端调用 Flask API，SQL 查询 MySQL

## 3. 存储分工

| 存储 | 用途 | 数据类型 |
|------|------|----------|
| MySQL | 业务数据 + 时序能耗 | 建筑、仪表、告警、用户、能耗记录 |
| Hive | 历史归档 | DWD/DWS 宽表 |

## 4. 模块说明

- **backend**：REST API，JWT 鉴权，MySQL SQL 查询封装
